...
public class ExcelReader {
    ...
    private static ExcelDataVO convertRowToData(Row row) {
        ...
        String Chinese = convertCellValueToString(cell);
        resultData.setChinese(Chinese);
        // 获取英文
        cell = row.getCell(cellNum++);
        String English = convertCellValueToString(cell);
        resultData.setEnglish(English);
        // 获取回答
        cell = row.getCell(cellNum++);
        String answer = convertCellValueToString(cell);
        if (null == answer || "".equals(answer)) {
            // 年龄为空
            resultData.setAnswer(null);
        } else {
