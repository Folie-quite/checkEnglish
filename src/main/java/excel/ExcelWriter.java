...
public class ExcelWriter {
    ...
    private static void convertDataToRow(ExcelDataVO data, Row row){
        ...
        cell.setCellValue(null == data.getChinese() ? "" : data.getChinese());
        // 年龄
        cell = row.createCell(cellNum++);
        if (null != data.getEnglish()) {
            cell.setCellValue(data.getEnglish());
        } else {
            cell.setCellValue("");
        }
        // 所在城市
        cell = row.createCell(cellNum++);
        cell.setCellValue(null == data.getAnswer() ? "" : data.getAnswer());
    }
}
